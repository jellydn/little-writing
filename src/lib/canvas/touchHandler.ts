...
/**
 * Pointer event handler functions
 */
...
/**
 * Internal state for pointer tracking
 */
...
export function createPointerHandlers(
   ...
   */
  function shouldRejectPointer(event: PointerEvent): boolean {
    // Palm rejection: ignore large contact areas
    if (
      event.width > CONFIG.PALM_REJECTION_SIZE &&
