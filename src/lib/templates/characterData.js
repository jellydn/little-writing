 ...
 */
function createStrokePath(id, path, startPoint, endPoint, guidePoints) {
    return {
        id,
        path,
        startPoint,
...
const CHARACTER_0 = {
    ...
    strokes: [
        createStrokePath(1, 'M 50 10 C 80 10 90 30 90 50 C 90 70 80 90 50 90 C 20 90 10 70 10 50 C 10 30 20 10 50 10', { x: 50, y: 10 }, { x: 50, y: 90 }, [
            { x: 50, y: 10 },
            { x: 65, y: 12 },
            { x: 80, y: 20 },
            { x: 88, y: 35 },
...
const CHARACTER_a = {
    ...
    strokes: [
        // Circle part
        createStrokePath(1, 'M 50 35 C 70 35 75 50 75 60 C 75 75 60 80 50 80 C 35 80 25 70 25 60 C 25 45 35 35 50 35', { x: 50, y: 35 }, { x: 50, y: 80 }, [
            { x: 50, y: 35 },
            { x: 58, y: 35 },
            { x: 68, y: 40 },
            { x: 75, y: 50 },
 ...
 */
export function getCharacter(character, category) {
    const characters = CHARACTERS_BY_CATEGORY[category];
    const template = characters.find((c) => c.character === character);
    if (!template) {
        throw new Error(`Character "${character}" not found in category "${category}"`);
    }
    return template;
}
/**
...
export function getTotalCharacterCount() {
    return ALL_CATEGORIES.reduce((total, category) => total + CHARACTERS_BY_CATEGORY[category].length, 0);
}
/**
 ...
 */
export function hasCharacter(character, category) {
    return CHARACTERS_BY_CATEGORY[category].some((c) => c.character === character);
}
/**
 ...
 */
export function getNextCharacter(currentCharacter, category) {
    const characters = CHARACTERS_BY_CATEGORY[category];
    const currentIndex = characters.findIndex((c) => c.character === currentCharacter);
 ...
 */
export function getPreviousCharacter(currentCharacter, category) {
    const characters = CHARACTERS_BY_CATEGORY[category];
    const currentIndex = characters.findIndex((c) => c.character === currentCharacter);
