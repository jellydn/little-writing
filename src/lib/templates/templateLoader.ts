...
export function validateTemplate(
  ...
  if (
    ...
    !['number', 'uppercase', 'lowercase'].includes(t.category as string)
  ) {
    return false;
  }
  ...
  if (
    ...
    typeof (t.bounds as Record<string, unknown>).width !== 'number' ||
    typeof (t.bounds as Record<string, unknown>).height !== 'number' ||
    typeof (t.bounds as Record<string, unknown>).viewBox !== 'string'
  ) {
    return false;
  }
  ...
  for (const stroke of t.strokes) {
    if (
      ...
      typeof (stroke as Record<string, unknown>).id !== 'number' ||
      typeof (stroke as Record<string, unknown>).path !== 'string' ||
      !(stroke as Record<string, unknown>).startPoint ||
      typeof (stroke as Record<string, unknown>).startPoint !== 'object' ||
      !(stroke as Record<string, unknown>).endPoint ||
      typeof (stroke as Record<string, unknown>).endPoint !== 'object' ||
      !Array.isArray((stroke as Record<string, unknown>).guidePoints)
    ) {
      return false;
    }

    // Validate startPoint and endPoint structure
    const startPoint = (stroke as Record<string, unknown>).startPoint as Record<
      string,
      unknown
    >;
    const endPoint = (stroke as Record<string, unknown>).endPoint as Record<
      string,
      unknown
    >;
    ...
    const guidePoints = (stroke as Record<string, unknown>)
      .guidePoints as unknown[];
  ...
  try {
    ...
    if (!validateTemplate(rawData)) {
      throw new TemplateLoadError(
        `Invalid template structure for character "${character}" in category "${category}"`,
        character,
        category
      );
    }
    ...
    if (rawData.character !== character) {
      throw new TemplateLoadError(
        `Template character mismatch: expected "${character}" but got "${rawData.character}"`,
        character,
        category
      );
    }
    ...
    if (rawData.category !== category) {
      throw new TemplateLoadError(
        `Template category mismatch: expected "${category}" but got "${rawData.category}"`,
        character,
        category
      );
    }
    ...
    if (
      ...
      throw new TemplateLoadError(
        `Template file not found for character "${character}" in category "${category}"`,
        character,
        category
      );
    }
    ...
    if (error instanceof SyntaxError) {
      throw new TemplateLoadError(
        `Invalid JSON in template file for character "${character}" in category "${category}"`,
        character,
        category
      );
    }
    ...
    // Wrap unknown errors
    throw new TemplateLoadError(
      `Failed to load template for character "${character}" in category "${category}": ${
...
export async function loadCategory(
  ...
  const characterLists: Record<Category, string[]> = {
  ...
  const errors: Array<{ character: string; error: Error }> = [];
  ...
  if (templates.length === 0) {
    throw new TemplateLoadError(
      `No valid templates found for category "${category}". Errors: ${errors.map((e) => `${e.character} (${e.error.message})`).join(', ')}`,
      undefined,
      category
    );
  }
  ...
  if (errors.length > 0) {
    console.warn(
      `Failed to load some templates for category "${category}":`,
      errors.map((e) => e.character)
    );
  }
 ...
 */
export async function loadCharacterTemplates(
  characters: Array<{ character: string; category: Category }>
): Promise<CharacterTemplate[]> {
  const templates: CharacterTemplate[] = [];
  const errors: Array<{ character: string; category: Category; error: Error }> =
    [];

  // Load templates in parallel
  const results = await Promise.allSettled(
    characters.map(({ character, category }) =>
      loadCharacterTemplate(character, category)
    )
  );
  ...
  if (templates.length === 0) {
    throw new TemplateLoadError(
      `Failed to load all templates. Errors: ${errors.map((e) => `${e.character} (${e.error.message})`).join(', ')}`
    );
  }
  ...
  if (errors.length > 0) {
    console.warn(
      'Failed to load some templates:',
      errors.map((e) => `${e.character} (${e.category})`)
    );
  }
