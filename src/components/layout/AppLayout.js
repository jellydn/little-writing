...
export const AppLayout = () => {
    ...
    const navigateToCategorySelection = useAppStore((state) => state.navigateToCategorySelection);
    ...
    const renderScreen = () => {
        switch (currentScreen) {
            case 'category-selection':
                return _jsx(CategorySelectionScreen, { onSelectCategory: selectCategory });
            case 'character-selection':
                return (_jsx(CharacterSelectionScreen, { category: currentCategory, characters: characters, onSelectCharacter: selectCharacter, onBack: navigateToCategorySelection }));
            case 'tracing':
                ...
                return (_jsx(TracingScreen, { template: currentCharacter, session: session, onStrokeStart: startStroke, onStrokeMove: addStrokePoint, onStrokeEnd: endStroke, onNext: nextCharacter, onPrevious: previousCharacter, onBack: navigateToCategorySelection, onClear: clearSession }));
            default:
                // Ensure exhaustive check - TypeScript will error if new screens are added
                const _exhaustiveCheck = currentScreen;
                return _exhaustiveCheck;
        }
    };
    return (_jsxs("div", { className: "app-layout", children: [_jsx("a", { href: "#main-content", className: "skip-link", children: "Skip to main content" }), _jsx(ErrorBoundary, { children: _jsx("main", { id: "main-content", className: "app-container", children: renderScreen() }) })] }));
};
