 ...
 */
function templateToCanvasPoint(point, offsetX, offsetY, scale) {
    return {
 ...
 */
function canvasToTemplatePoint(point, offsetX, offsetY, scale) {
    return {
 ...
 */
function calculateScale(template, canvasWidth, canvasHeight) {
 ...
 */
function calculateOffset(template, canvasWidth, canvasHeight, scale) {
 ...
 */
function renderStroke(ctx, points, color, lineWidth) {
    if (points.length === 0)
        return;
    ctx.save();
...
export const Canvas = ({ template, session, width, height, onStrokeStart, onStrokeMove, onStrokeEnd, }) => {
    ...
    const renderCanvas = React.useCallback(() => {
        ...
        session.strokes.forEach((stroke) => {
            const strokeColor = stroke.isValid === null
                ? COLORS.highlight
                : stroke.isValid
                    ? COLORS.correct
                    : COLORS.incorrect;
            // Convert template points to canvas coordinates
            const canvasPoints = stroke.points.map((p) => templateToCanvasPoint(p, offset.x, offset.y, scale));
            renderStroke(ctx, canvasPoints, strokeColor, UI_CONFIG.USER_STROKE_WIDTH);
        });
    ...
    const adjustedCallbacks = React.useMemo(() => ({
        onStrokeStart: (point) => {
            const adjustedPoint = canvasToTemplatePoint(point, offset.x, offset.y, scale);
    ...
    return (_jsx("canvas", { ref: canvasRef, width: width, height: height, "aria-label": `Drawing canvas for tracing ${template.displayName || template.character}`, role: "img", "aria-describedby": `canvas-instructions-${template.character}`, style: {
            width: `${width}px`,
            height: `${height}px`,
            ...
            display: 'block',
        }, children: _jsxs("span", { id: `canvas-instructions-${template.character}`, className: "visually-hidden", children: ["Use your finger or stylus to trace the ", template.displayName || template.character, " character on the screen. Follow the guide lines from start to finish."] }) }));
};
