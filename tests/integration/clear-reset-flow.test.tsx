...
describe('Clear/Reset Flow Integration', () => {
  ...
  it('should render Clear button on tracing screen', async () => {
    ...
    const clearButton = screen.getByLabelText('Clear drawing and start over');
  ...
  it('should preserve character template when clearing session', async () => {
    ...
    await waitFor(() => {
      expect(useAppStore.getState().session?.template).toEqual(originalTemplate);
      expect(useAppStore.getState().currentCharacter).toEqual(originalCharacter);
    });
  });
  ...
  it('should disable Next button after clearing completed session', async () => {
    render(<AppLayout />);
