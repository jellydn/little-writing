...
describe('Navigation Flow Integration', () => {
  ...
  it('should enable Next button after completing character', async () => {
    render(<AppLayout />);

    // Navigate to character "0" and complete it
    act(() => {
    ...
    // Next button should be enabled when session is complete
    await waitFor(() => {
      expect(screen.getByLabelText('Next character')).not.toBeDisabled();
    });
  });

  it('should disable Next button when character is not completed', async () => {
    render(<AppLayout />);
    ...
    // Next button should be disabled when session is not complete
    const nextButton = screen.getByLabelText('Next character');
    expect(nextButton).toBeDisabled();
  });
        ...
        strokes: [
          {
            ...
            endPoint: { x: 50, y: 80 },
            guidePoints: [{ x: 50, y: 20 }, { x: 50, y: 80 }],
          },
        ],
        totalStrokes: 1,
      });
  ...
  it('should maintain category when navigating between characters', async () => {
    ...
    const uppercaseButton = screen.getByText('Letters');
    ...
    // Select character 'A'
    const characterA = screen.getByText('A');
